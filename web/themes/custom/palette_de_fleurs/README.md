...
`./palette_de_fleurs/bootstrap`.

If for whatever reason you have an additional `bootstrap` directory wrapping the
first `bootstrap` directory (e.g. `./palette_de_fleurs/bootstrap/bootstrap`), remove the
wrapping `bootstrap` directory. You will only ever need to touch these files if
or when you upgrade your version of the [Bootstrap Framework].

{.alert.alert-warning} **WARNING:** Do not modify the files inside of
`./palette_de_fleurs/bootstrap` directly. Doing so may cause issues when upgrading the
[Bootstrap Framework] in the future.

## Overrides {#overrides}
The `./palette_de_fleurs/scss/_default-variables.scss` file is generally where you will
...
The `./palette_de_fleurs/scss/overrides.scss` file contains various Drupal overrides to
properly integrate with the [Bootstrap Framework]. It may contain a few
enhancements, feel free to edit this file as you see fit.

The `./palette_de_fleurs/scss/style.scss` file is the glue that combines:
...
that you should compile to `./palette_de_fleurs/css/style.css` (note the same file
